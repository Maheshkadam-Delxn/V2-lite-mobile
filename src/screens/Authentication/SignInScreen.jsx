...
export default function LoginScreen() {
  ...
  const [scaleAnim] = useState(new Animated.Value(0.8)); // scale animation
  ...
  // Animate modal when visible and auto navigate after 2s
  useEffect(() => {
    if (modalVisible) {
      Animated.parallel([
        ...
        <View className="flex-1 px-6 pt-6" style={{ minHeight: height }}>
          ...
          >
            <View className="p-6 flex-1">
              ...
              {/* Divider line just above sign-in button */}
              <View className="w-full h-px bg-gray-200 mt-6" />

              {/* Sign-in button at the very bottom */}
              <View className="mt-6">
                <TouchableOpacity
